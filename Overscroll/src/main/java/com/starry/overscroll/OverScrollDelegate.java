...
import com.starry.overscroll.PathScroller.PathPointsHolder;
...
public class OverScrollDelegate {
	...
	public static abstract class OverScrollStyle {
		...
		public static final float DEFAULT_DRAW_TRANSLATE_RATE = 0.3f;
	...
	private boolean onTouchEventInternal(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (!isOsDrag()) {
				...
				if ((Math.abs(yDiff) >= mTouchSlop)) {// mTouchSlop
					boolean isOs = false;
					if (!canScrollUp && yDiff > 0) {
						setState(OS_DRAG_TOP);
						isOs = true;
